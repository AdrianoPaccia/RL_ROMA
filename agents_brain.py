...
class Agents():
    ...
    def __init__(self,custom_env,vs):  
        ...
        #cheats
        self.ability_casted = [False,False,False,False,False,False]
        self.ability_counter = [0,0,0,0,0,0]
        self.ability_threshold = 5
        self.abuse_ability_penalty = -1
        self.still_penalty = -0.5

        #cazzate
        self.hitpoints = np.zeros(2)
    ...
    def make_step(self, observation_n, last_action_n, exploit = [True,True] ):
        ...
        #cheat = self.cast_cheats(action_n_id,reward_n,done_n)
        #reward_n_cheated = reward_n + cheat
    ...
    def cast_cheats(self,action_n_id,reward_n,done_n):
        cheat = np.zeros(6)
        for i in range(6):
            if self.ability_casted[i]:
                if reward_n[i]== 0.0:
                    self.ability_counter[i]+=1
                else:
                    self.ability_counter[i]=0
                    self.ability_casted[i] = False
            if action_n_id[i]==13:
                self.ability_casted[i]=True

            if self.ability_counter[i] > self.ability_threshold:
                cheat[i] = self.abuse_ability_penalty
                self.ability_counter[i]=0
                self.ability_casted[i] = False
        ...
        if (self.ability_casted[0:3] == [False,False,False]):
            cheat[0]+=self.still_penalty
            cheat[1]+=self.still_penalty
            cheat[2]+=self.still_penalty
        if (self.ability_casted[3:6] == [False,False,False]):
            cheat[3]+=self.still_penalty
            cheat[4]+=self.still_penalty
            cheat[5]+=self.still_penalty
    ...
    def train(self,max_steps=5,episodes=5):
        start = time.time()
        end_old = start
        print("\nSTART training")
        ...
        for epochs in range(self.training_epochs):
            ...
            for ep in range(episodes):
                ...
                for r_i in range(max_steps):
                    ...
                    self.roll_in_episode("roma")
                if self.match[0] == 'roma':
                    d_loss, c_loss,TD_loss = self.agent_1.update(self.buffer,self.episode_limit)
                    print(" - Dissimilarity loss: ",d_loss )
                    print(" - Cross Entropy loss: ",c_loss )
                    print(" - TD error loss: ",TD_loss )
                    loss =  d_loss+ c_loss+TD_loss
                else:
            ...
            if not self.ghost:
                for ep in range(episodes):
                    ...
                    if self.match[1] == 'roma':
                        d_loss, c_loss,TD_loss = self.agent_2.update(self.buffer,self.episode_limit)
                        print(" - Dissimilarity loss: ",d_loss )
                        print(" - Cross Entropy loss: ",c_loss )
                        print(" - TD error loss: ",TD_loss )
                        loss =  d_loss+ c_loss+TD_loss
            ...
            print('\nEpoch duration:',str(datetime.timedelta(seconds=duration)))
            end_old = end
            print('\n***************** SMACK DOWN *****************')
            self.evaluation()
            print('**********************************************')
    ...
    def evaluation(self):
        ...
        while not done:
            ...
            if reward_n[0]>0 or reward_n[1]>0 or reward_n[2]>0 or reward_n[3]<0 or reward_n[4]<0 or reward_n[5]<0:
                print('MERDA colpito')
            if reward_n[3]>0 or reward_n[4]>0 or reward_n[5]>0 or reward_n[0]<0 or reward_n[1]<0 or reward_n[2]<0 :
                print('ROMANISTA colpito')
